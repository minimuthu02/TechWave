    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', async function () {
            ...
            if (confirm('Are you sure you want to delete this blog?')) {
                try {
                    const response = await fetch(`/blog-application/api/blog/delete.php?id=${blogId}`, {
                        method: 'DELETE'
                    });
...
async function saveBlog(blogData, isUpdate = false) {
    const endpoint = isUpdate
        ? '/blog-application/api/blog/update.php'
        : '/blog-application/api/blog/create.php';
    ...
    try {
        ...
        if (result.success) {
            ...
            window.location.href = `/blog-application/pages/view-blog.php?id=${redirectId}`;
        } else {
